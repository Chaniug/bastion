# Bastion WebDAV 备份格式规范

## 1. 概述
Bastion 的 WebDAV 备份机制会将用户选定的数据打包成一个 ZIP 压缩包（`.zip` 或 加密的 `.enc.zip`）。该包内部仅包含 `.json` 数据文件、`.csv` 汇总文件和媒体资源文件的集合，结构清晰，便于未来的跨平台解析。
...
├── Bastion_20240101_password.csv # 密码汇总(兼容用)
├── Bastion_20240101_totp.csv     # TOTP汇总
...
3.  生成的文件扩展名为 `.enc.zip`。
4.  文件头会包含 `BASTION_ENC_V1` 标识、Salt 和 IV。
