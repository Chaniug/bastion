# Bastion Android 本地存储与加密技术文档

## 1. 概述
Bastion Android 采用多层加密架构，确保用户的敏感信息（如密码、信用卡号、证件信息等）在本地存储和备份过程中的绝对安全。系统核心遵循“零知识保护”原则，即使设备丢失或数据库文件被提取，攻击者在没有主密码的情况下也无法获取明文数据。

## 2. 本地数据存储
Bastion 使用 Android 官方推荐的 **Room Persistence Library** (基于 SQLite) 进行结构化数据存储。

*   **主要数据库文件**: `password_database.db`
*   **敏感字段处理**: 数据库中的敏感字段（如 `PasswordEntry` 的 `password` 字段）**不存储明文**。在存入数据库前，必须经过加密层处理；在读取时，经过解密层恢复。
*   **存储位置**: 应用私有目录 (`/data/data/com.bastion.app/databases/`)，外部应用无权访问。
...
## 5. 备份加密 (WebDAV)
在进行 WebDAV 备份时，Bastion 会生成一个 ZIP 压缩包，并根据用户设置提供额外加密：
*   **算法**: AES-256-GCM。
*   **派生算法**: 同样使用 PBKDF2WithHmacSHA256 (100,000 迭代)。
*   **文件格式**: 包含固定的标识位符 (`BASTION_ENC_V1`)，以及随机生成的盐值和 IV，确保即使两个用户使用相同的备份密码，生成的备份文件也完全不同。

## 6. 总结
Bastion Android 结合了 Android 系统硬件级安全特性 (KeyStore) 与行业标准加密算法 (AES-GCM, PBKDF2)，实现了银行级的本地数据保护方案。
