...
> Bastion’s local password vault client for Android  
...
Bastion for Android is a good fit for users who:
...
- **MDBX local format**: Bastion’s self-developed nested encrypted database format with version history and conflict detection
...
- **Dependency injection**: Koin (application starts in `BastionApplication`)
...
- `takagi/ru/bastion/ui`: Compose pages and UI components
- `takagi/ru/bastion/data`: Room entities, DAO, database migrations
- `takagi/ru/bastion/repository`: data access encapsulation and aggregation
- `takagi/ru/bastion/security`: encryption, keys, and authentication
- `takagi/ru/bastion/bitwarden`: Bitwarden API, encryption, sync, and ViewModel logic
- `takagi/ru/bastion/autofill`: AutoFill services and flows
- `takagi/ru/bastion/passkey`: Android 14+ Credential Provider integration
- `takagi/ru/bastion/workers`: background tasks such as automated WebDAV backup
...
Bastion for Android is open-sourced under the [GNU General Public License v3.0](LICENSE).
