...
> Bastion’s local-first password vault client for iOS  
...
::: note Abstract
Bastion for iOS is the native iOS client in the Bastion password vault ecosystem. It follows a <mark>local-first</mark> philosophy and focuses on delivering a secure, controllable, and recoverable password management experience on iPhone.

Key capabilities: `MDBX Vault`, `AutoFill`, `TOTP`, `WebDAV`.
:::

Bastion for iOS is the native iOS client in the Bastion password vault ecosystem. It follows a **local-first** design philosophy and focuses on providing a secure, controllable, and recoverable password management experience on iPhone.
...
Bastion for iOS is a good fit for users who:

- Want a **local-first** password management solution and do not want core credentials entrusted to third-party services.
- Already manage passwords, TOTP, private notes, and related data in the Bastion Android or MDBX ecosystem.
...
- Compatible with Bastion / MDBX data model design
...
- **Core logic**: `BastionCore` (TOTP, `otpauth://` parsing, secure password generation)
- **Storage layer**: `BastionStorage` (vault repository, KDBX compatibility, AutoFill index)
- **Security layer**: `BastionSecurity` (Keychain / LocalAuthentication boundary)
- **Sync layer**: `BastionSync` (WebDAV, OneDrive, Bitwarden adapters)
...
Note: do not run `Scripts/build-mdbx-xcframework.sh` in parallel with an Xcode build. The script replaces `BastionMDBXGenerated.xcframework` and may cause Xcode reads to fail.
...
Bastion’s design, compatibility work, and parts of its feature direction were inspired and supported by the following excellent open-source projects and software:
...
Bastion for iOS is open-sourced under the [GNU General Public License v3.0](LICENSE).
