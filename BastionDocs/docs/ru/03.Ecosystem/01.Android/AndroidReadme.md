...
> Android-клиент локального хранилища паролей Bastion  
...
Bastion for Android подходит пользователям, которые:
...
- **Локальный формат MDBX**: собственный вложенный зашифрованный формат базы данных Bastion с историей версий и обнаружением конфликтов
...
- **DI**: Koin (запуск в `BastionApplication`)
...
- `takagi/ru/bastion/ui`: Compose-страницы и UI-компоненты
- `takagi/ru/bastion/data`: сущности Room, DAO, миграции базы данных
- `takagi/ru/bastion/repository`: инкапсуляция и агрегирование доступа к данным
- `takagi/ru/bastion/security`: шифрование, ключи и аутентификация
- `takagi/ru/bastion/bitwarden`: Bitwarden API, шифрование, синхронизация и логика ViewModel
- `takagi/ru/bastion/autofill`: сервисы и сценарии AutoFill
- `takagi/ru/bastion/passkey`: интеграция Credential Provider для Android 14+
- `takagi/ru/bastion/workers`: фоновые задачи, например автоматический WebDAV backup
...
Bastion for Android распространяется как open source по [GNU General Public License v3.0](LICENSE).
