...
> Bastion by Avalonia: local-first кроссплатформенное хранилище паролей на Avalonia, .NET и MDBX.  
...
::: note Кратко
Bastion for Windows — desktop-реализация хранилища паролей Bastion, сфокусированная на local-first подходе, управлении паролями и совместимости MDBX vault. Документ оформлен в стиле README и содержит обзор функций, стек, архитектуру и сведения для разработки.
:::

Bastion for Windows переносит local-first и security-first направление Bastion на desktop-платформы. Приложение предоставляет управление паролями, TOTP, приватными заметками, банковскими картами и документами, а также поддерживает локальное шифрование, импорт/экспорт, резервное копирование и совместимость MDBX vault.

---

## Позиционирование проекта

Bastion for Windows — local-first клиент хранилища паролей для desktop-пользователей. Он построен на кроссплатформенном desktop UI Avalonia, .NET 10 и локальном MDBX vault, чтобы дать современный и расширяемый опыт управления паролями на desktop.

Ключевые цели:

- Предоставить локальное зашифрованное хранилище паролей и управление TOTP
- Поддержать совместимость с KeePass `.kdbx` и данными Bastion / MDBX
...
- Импорт/экспорт: Bastion JSON, password CSV, TOTP CSV, Aegis JSON и другие форматы
- Синхронизация и backup: WebDAV / OneDrive backup и restore
- MDBX vault: создание, проверка и управление локальными базами Bastion MDBX-1
...
| MDBX | Rust MDBX workspace, UniFFI, `mdbx_ffi.dll` | Переиспользование ядра Bastion MDBX vault |
...
Архитектуру Bastion for Windows можно представить как дерево мышления desktop-хранилища паролей, где:
...
- `Bastion.App`: Avalonia UI, окна и desktop-сервисы
- `Bastion.Core`: доменные модели, криптография, TOTP, импорт/экспорт, генерация паролей
- `Bastion.Data`: локальная база, vault storage, миграции и MDBX backend repositories
- `Bastion.Platform`: кроссплатформенный адаптерный слой с WebDAV, OneDrive, KeePass и MDBX bridge
...
- `bastion by avalonia/src/Bastion.App`: вход Avalonia-приложения, главное окно, ViewModel и desktop UI-сервисы
- `bastion by avalonia/src/Bastion.Core`: core-модели, криптография, TOTP, генерация паролей, импорт/экспорт и безопасность
- `bastion by avalonia/src/Bastion.Data`: SQLite database, Dapper repositories, миграции, vault storage и MDBX backend repositories
- `bastion by avalonia/src/Bastion.Platform`: platform adapters, WebDAV, OneDrive, KeePass, Windows Secret Protector и native bridge MDBX UniFFI
- `bastion by avalonia/tests/Bastion.Tests`: тесты core-сервисов, repositories, MDBX integration и platform-сервисов

---

## Интеграция MDBX-1

Bastion for Windows поддерживает MDBX-1 local-first vault. Это не обычная SQLite-таблица паролей, а локальный формат базы данных с историей версий, обработкой конфликтов, восстановлением snapshot и границами безопасности.
...
## Связь с Bastion / MDBX

- Bastion: дает продуктовую идею, local-first направление и UX-ориентир для управления паролями
- MDBX: дает local-first формат vault и долгосрочно поддерживаемую структуру данных
- Bastion for Windows: объединяет оба направления в desktop-реализации для Windows

---

## Благодарности

Bastion for Windows ориентируется на следующие проекты и заимствует у них идеи:
...
- [MDBX](https://github.com/Bastion/Mdbx) - ориентир local-first vault format
