...
::: note 摘要
Bastion for iOS 是 Bastion 密码库生态中的原生 iOS 客户端，采用 <mark>本地优先</mark> 的设计理念，专注于在 iPhone 上提供安全、可控、可恢复的密码管理体验。 ✨

主要特性：`MDBX Vault`、`AutoFill`、`TOTP`、`WebDAV`。
:::

Bastion for iOS 是 Bastion 密码库生态中的原生 iOS 客户端，采用 **本地优先** 的设计理念，专注于在 iPhone 上提供安全、可控、可恢复的密码管理体验。
...
Bastion for iOS 适合以下用户：

- 希望使用**本地优先**方案管理密码，不愿将核心凭据托管给第三方服务。
- 已经在 Bastion Android 或 MDBX 生态中管理密码、TOTP、私密笔记等数据。
...
- **App 层**：<mark>SwiftUI</mark>、Observation、AuthenticationServices、LocalAuthentication、Keychain、WidgetKit
- **本地 Vault**：MDBX (Rust) + UniFFI → `BastionMDBX` (Swift)
- **核心逻辑**：`BastionCore`（TOTP、`otpauth://` 解析、安全密码生成）
- **存储层**：`BastionStorage`（Vault repository、KDBX 兼容、AutoFill 索引）
- **安全层**：`BastionSecurity`（Keychain / LocalAuthentication 边界）
- **同步层**：`BastionSync`（WebDAV、OneDrive、Bitwarden 适配）
...
注意：不要并行运行 `Scripts/build-mdbx-xcframework.sh` 与 Xcode build。脚本会替换 `BastionMDBXGenerated.xcframework`，并可能导致 Xcode 读取失败。
...
Bastion 的设计、兼容性适配与部分功能方向，受到了以下优秀开源项目和软件的启发与帮助：
...
Bastion for iOS 基于 [GNU General Public License v3.0](LICENSE) 开源发布。
