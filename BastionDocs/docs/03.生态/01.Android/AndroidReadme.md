...
::: note 摘要
Bastion for Android 是 Bastion 密码库生态中的原生 Android 客户端，采用 <mark>本地优先</mark> 的设计理念，专注于在 Android 手机上提供安全、可控、可恢复的密码管理体验。 ✨

主要特性：`本地 Vault`、`Bitwarden 聚合`、`KeePass 兼容`、`AutoFill`、`TOTP`、`WebDAV`。
:::

Bastion for Android 是聚合 **Bitwarden** 与 **KeePass** 的本地密码库（Local Vault）。它以本地存储优先为核心，帮助你在 Android 手机上统一管理账号密码、2FA、私密笔记与敏感附件，同时支持可选的 WebDAV 备份恢复与跨平台数据同步。
...
- **MDBX 本地格式**：Bastion 自研的嵌套加密数据库格式，支持版本历史与冲突检测
...
- **依赖注入**：Koin（应用启动于 `BastionApplication`）
...
- `takagi/ru/bastion/ui`：Compose 页面与组件
- `takagi/ru/bastion/data`：Room 实体、DAO、数据库迁移
- `takagi/ru/bastion/repository`：数据访问封装与聚合
- `takagi/ru/bastion/security`：加密、密钥与鉴权
- `takagi/ru/bastion/bitwarden`：Bitwarden API、加密、同步、ViewModel
- `takagi/ru/bastion/autofill`：AutoFill 服务与流程
- `takagi/ru/bastion/passkey`：Android 14+ Credential Provider 集成
- `takagi/ru/bastion/workers`：后台任务（如自动 WebDAV 备份）
...
Bastion for Android 基于 [GNU General Public License v3.0](LICENSE) 开源发布。
