...
> Bastion by Avalonia：用 Avalonia、.NET 与 MDBX 打造本地优先的跨平台密码库。  
...
::: note 摘要
Bastion for Windows 是 Bastion 密码库的桌面端实现，聚焦本地优先、密码管理与 MDBX vault 兼容。本文按项目 README 风格撰写，提供功能介绍、技术栈、架构视图与开发说明。
:::

Bastion for Windows 目标是在桌面平台上延续 Bastion 的本地优先与安全优先路线，提供密码、TOTP、私密笔记、银行卡与证件管理，并支持本地加密、导入导出、备份与 MDBX vault 兼容。

---

## 项目定位

Bastion for Windows 是面向桌面用户的本地优先密码库客户端。它以跨平台的 Avalonia 桌面 UI 为基础，结合 .NET 10 与 MDBX 本地 vault，构建一个既现代又可扩展的桌面密码管理体验。

本项目核心目标：

- 提供本地加密密码库与 TOTP 管理能力
- 支持 KeePass `.kdbx` 与 Bastion / MDBX 数据兼容
...
- 导入导出：支持 Bastion JSON、密码 CSV、TOTP CSV、Aegis JSON 等格式
- 同步与备份：WebDAV / OneDrive 备份与恢复能力
- MDBX vault：创建、检查与管理 Bastion MDBX-1 本地数据库
...
| MDBX 接入 | Rust MDBX workspace, UniFFI, `mdbx_ffi.dll` | 复用 Bastion MDBX vault 核心能力 |
...
::: tip 架构预览
Bastion for Windows 的架构是一颗“桌面密码库思维树”，核心在于：
- UI 层负责交互与展示
- Core 层负责业务与加密逻辑
...
::: details 架构说明
- `Bastion.App`：Avalonia 界面、窗口与桌面服务
- `Bastion.Core`：业务模型、加密、TOTP、导入导出、密码生成
- `Bastion.Data`：本地数据库、Vault 存储、迁移与 MDBX 后端仓储
- `Bastion.Platform`：跨平台适配层，包含 WebDAV、OneDrive、KeePass、MDBX bridge
...
- `bastion by avalonia/src/Bastion.App`：Avalonia 应用入口、主窗口、ViewModel 与桌面 UI 服务
- `bastion by avalonia/src/Bastion.Core`：核心模型、加密、TOTP、密码生成、导入导出、安全能力
- `bastion by avalonia/src/Bastion.Data`：SQLite 数据库、Dapper 仓储、迁移、Vault 存储、MDBX 后端仓储
- `bastion by avalonia/src/Bastion.Platform`：平台适配、WebDAV、OneDrive、KeePass、Windows Secret Protector、MDBX UniFFI 本地桥接
- `bastion by avalonia/tests/Bastion.Tests`：核心服务、仓储、MDBX 接入与平台服务测试

---

## MDBX-1 接入

Bastion for Windows 支持 MDBX-1 本地优先 vault，它不是普通的 SQLite 密码表，而是一个带有版本历史、冲突处理、快照恢复与安全边界的本地数据库格式。
...
- Bastion：提供产品理念、本地优先路线与密码管理体验参考
- MDBX：提供本地优先 vault 格式与长期可维护的数据结构
- Bastion for Windows：作为桌面端实现，将两者能力融合到 Windows 桌面应用中

---

## 致谢

Bastion for Windows 参考并借鉴了以下项目：
...
- [MDBX](https://github.com/Bastion/Mdbx)：本地优先 vault 格式参考
