...
> Bastion by Avalonia: kho mật khẩu đa nền tảng ưu tiên cục bộ được xây dựng bằng Avalonia, .NET và MDBX.  
...
::: note Tóm tắt
Bastion for Windows là bản triển khai desktop của kho mật khẩu Bastion, tập trung vào ưu tiên cục bộ, quản lý mật khẩu và tương thích MDBX vault. Tài liệu này theo phong cách README, cung cấp giới thiệu tính năng, công nghệ, kiến trúc và ghi chú phát triển.
:::

Bastion for Windows hướng tới việc đưa lộ trình ưu tiên cục bộ và ưu tiên bảo mật của Bastion lên nền tảng desktop, cung cấp quản lý mật khẩu, TOTP, ghi chú riêng tư, thẻ ngân hàng và giấy tờ định danh, đồng thời hỗ trợ mã hóa cục bộ, nhập/xuất, sao lưu và tương thích MDBX vault.

---

## Định vị dự án

Bastion for Windows là ứng dụng khách kho mật khẩu ưu tiên cục bộ dành cho người dùng desktop. Dựa trên UI desktop đa nền tảng Avalonia, kết hợp .NET 10 và MDBX local vault, dự án xây dựng trải nghiệm quản lý mật khẩu desktop hiện đại và có thể mở rộng.

Mục tiêu cốt lõi:

- Cung cấp kho mật khẩu mã hóa cục bộ và năng lực quản lý TOTP
- Hỗ trợ tương thích KeePass `.kdbx` và dữ liệu Bastion / MDBX
...
- Nhập và xuất: hỗ trợ Bastion JSON, password CSV, TOTP CSV, Aegis JSON và các định dạng khác
- Đồng bộ và sao lưu: sao lưu/khôi phục WebDAV / OneDrive
- MDBX vault: tạo, kiểm tra và quản lý cơ sở dữ liệu cục bộ Bastion MDBX-1
...
| Tích hợp MDBX | Rust MDBX workspace, UniFFI, `mdbx_ffi.dll` | Tái sử dụng năng lực lõi Bastion MDBX vault |
...
Kiến trúc Bastion for Windows có thể hiểu như một cây tư duy của kho mật khẩu desktop, với trọng tâm:
...
- `Bastion.App`: giao diện Avalonia, cửa sổ và dịch vụ desktop
- `Bastion.Core`: mô hình nghiệp vụ, mã hóa, TOTP, nhập/xuất, tạo mật khẩu
- `Bastion.Data`: cơ sở dữ liệu cục bộ, lưu trữ vault, di trú và repository backend MDBX
- `Bastion.Platform`: lớp adapter đa nền tảng gồm WebDAV, OneDrive, KeePass và MDBX bridge
...
- `bastion by avalonia/src/Bastion.App`: entry point Avalonia, cửa sổ chính, ViewModel và dịch vụ UI desktop
- `bastion by avalonia/src/Bastion.Core`: mô hình lõi, mã hóa, TOTP, tạo mật khẩu, nhập/xuất và năng lực bảo mật
- `bastion by avalonia/src/Bastion.Data`: cơ sở dữ liệu SQLite, repository Dapper, di trú, lưu trữ vault và repository backend MDBX
- `bastion by avalonia/src/Bastion.Platform`: adapter nền tảng, WebDAV, OneDrive, KeePass, Windows Secret Protector và cầu nối native MDBX UniFFI
- `bastion by avalonia/tests/Bastion.Tests`: kiểm thử dịch vụ lõi, repository, tích hợp MDBX và dịch vụ nền tảng

---

## Tích hợp MDBX-1

Bastion for Windows hỗ trợ MDBX-1 local-first vault. Đây không phải là bảng mật khẩu SQLite thông thường, mà là định dạng cơ sở dữ liệu cục bộ có lịch sử phiên bản, xử lý xung đột, khôi phục snapshot và ranh giới bảo mật.
...
## Quan hệ với Bastion / MDBX

- Bastion: cung cấp理念 sản phẩm, lộ trình ưu tiên cục bộ và tham khảo trải nghiệm quản lý mật khẩu
- MDBX: cung cấp định dạng vault ưu tiên cục bộ và cấu trúc dữ liệu có thể bảo trì lâu dài
- Bastion for Windows: triển khai desktop kết hợp năng lực của cả hai trên Windows

---

## Lời cảm ơn

Bastion for Windows tham khảo và lấy cảm hứng từ các dự án sau:
...
- [MDBX](https://github.com/Bastion/Mdbx) - tham khảo định dạng vault ưu tiên cục bộ
