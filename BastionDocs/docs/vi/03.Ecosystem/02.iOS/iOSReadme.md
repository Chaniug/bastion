...
> Ứng dụng khách kho mật khẩu iOS ưu tiên cục bộ của Bastion  
...
::: note Tóm tắt
Bastion for iOS là ứng dụng khách iOS gốc trong hệ sinh thái kho mật khẩu Bastion. Ứng dụng theo triết lý <mark>ưu tiên cục bộ</mark>, tập trung cung cấp trải nghiệm quản lý mật khẩu an toàn, có thể kiểm soát và có thể khôi phục trên iPhone.

Năng lực chính: `MDBX Vault`, `AutoFill`, `TOTP`, `WebDAV`.
:::

Bastion for iOS là ứng dụng khách iOS gốc trong hệ sinh thái kho mật khẩu Bastion. Ứng dụng theo triết lý thiết kế **ưu tiên cục bộ**, tập trung cung cấp trải nghiệm quản lý mật khẩu an toàn, có thể kiểm soát và có thể khôi phục trên iPhone.
...
Bastion for iOS phù hợp với người dùng:

- Muốn dùng giải pháp quản lý mật khẩu **ưu tiên cục bộ** và không muốn giao dữ liệu đăng nhập cốt lõi cho dịch vụ bên thứ ba.
- Đã quản lý mật khẩu, TOTP, ghi chú riêng tư và dữ liệu liên quan trong Bastion Android hoặc hệ sinh thái MDBX.
...
- Tương thích với tư duy mô hình dữ liệu Bastion / MDBX
...
- **Logic cốt lõi**: `BastionCore` (TOTP, phân tích `otpauth://`, tạo mật khẩu an toàn)
- **Lớp lưu trữ**: `BastionStorage` (vault repository, tương thích KDBX, chỉ mục AutoFill)
- **Lớp bảo mật**: `BastionSecurity` (ranh giới Keychain / LocalAuthentication)
- **Lớp đồng bộ**: `BastionSync` (WebDAV, OneDrive, adapter Bitwarden)
...
Lưu ý: không chạy song song `Scripts/build-mdbx-xcframework.sh` với Xcode build. Script sẽ thay thế `BastionMDBXGenerated.xcframework` và có thể khiến Xcode đọc thất bại.
...
Bastion for iOS được phát hành mã nguồn mở theo [GNU General Public License v3.0](LICENSE).
