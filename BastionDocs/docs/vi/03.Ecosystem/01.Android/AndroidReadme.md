...
> Ứng dụng khách Android cho kho mật khẩu cục bộ của Bastion  
...
::: note Tóm tắt
Bastion for Android là ứng dụng khách Android gốc trong hệ sinh thái kho mật khẩu Bastion. Ứng dụng theo triết lý <mark>ưu tiên cục bộ</mark>, tập trung cung cấp trải nghiệm quản lý mật khẩu an toàn, có thể kiểm soát và có thể khôi phục trên điện thoại Android.

Năng lực chính: `Local Vault`, `tích hợp Bitwarden`, `tương thích KeePass`, `AutoFill`, `TOTP`, `WebDAV`.
:::

Bastion for Android là kho mật khẩu cục bộ kết hợp **Bitwarden** và **KeePass**. Với lưu trữ cục bộ làm trọng tâm, ứng dụng giúp bạn quản lý tài khoản, mật khẩu, 2FA, ghi chú riêng tư và tệp đính kèm nhạy cảm trên Android, đồng thời hỗ trợ sao lưu, khôi phục và đồng bộ dữ liệu đa nền tảng qua WebDAV khi cần.
...
Bastion for Android phù hợp với người dùng:
...
- **Định dạng MDBX cục bộ**: định dạng cơ sở dữ liệu mã hóa lồng nhau do Bastion phát triển, hỗ trợ lịch sử phiên bản và phát hiện xung đột
...
- **Dependency injection**: Koin (khởi động trong `BastionApplication`)
...
- `takagi/ru/bastion/ui`: trang Compose và thành phần UI
- `takagi/ru/bastion/data`: entity Room, DAO, di trú cơ sở dữ liệu
- `takagi/ru/bastion/repository`: đóng gói truy cập dữ liệu và tích hợp
- `takagi/ru/bastion/security`: mã hóa, khóa và xác thực
- `takagi/ru/bastion/bitwarden`: API Bitwarden, mã hóa, đồng bộ và logic ViewModel
- `takagi/ru/bastion/autofill`: dịch vụ và luồng AutoFill
- `takagi/ru/bastion/passkey`: tích hợp Credential Provider cho Android 14+
- `takagi/ru/bastion/workers`: tác vụ nền như sao lưu WebDAV tự động
...
Thiết kế, khả năng tương thích và một phần định hướng tính năng của Bastion được truyền cảm hứng và hỗ trợ bởi các dự án/phần mềm mã nguồn mở sau:
...
Bastion for Android được phát hành mã nguồn mở theo [GNU General Public License v3.0](LICENSE).
