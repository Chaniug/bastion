...
class BitwardenAuthService(
            ...
            logDiag(
                ...
                message =
                    "serverClass=${classifyServer(urls.vault)}, vault=${urls.vault}, identity=${urls.identity}, api=${urls.api}, " +
                        ...
                        "captchaProvided=${!normalizedCaptcha.isNullOrBlank()}, desktopJvm=true, " +
    ...
    suspend fun loginNewDeviceOtp(
        ...
        try {
            ...
            if (response.isSuccessful) {
                ...
                Logger.e(TAG, "New device login failed: ${response.code()} - $errorBody")
                logDiag(
                    flow = "new_device",
                    attemptId = attemptId,
                    stage = "result_error",
    ...
    /**
     * 获取设备 ID（桌面端持久化到数据目录文件）
     */
    private fun getDeviceId(): String {
        val deviceFile = java.io.File(com.bastion.app.platform.PathProvider.resolve("bitwarden_device_id.txt"))
        val existing = runCatchingObserved { deviceFile.readText().trim() }.getOrNull()
        if (!existing.isNullOrBlank()) return existing
        val newId = UUID.randomUUID().toString()
        runCatchingObserved { deviceFile.writeText(newId) }
        return newId
    }
     ...
     * 获取设备名称
     * 
     * 使用桌面端设备名称，因为现在使用 desktop 客户端类型
     */
    private fun getDeviceName(): String {
        return "Bastion Desktop"
    }

    private fun toBase64UrlNoPadding(value: String): String {
        val base64 = Base64.encodeToString(value.toByteArray(Charsets.UTF_8))
        return base64
            .replace('+', '-')
            .replace('/', '_')
            .replace("=", "")
