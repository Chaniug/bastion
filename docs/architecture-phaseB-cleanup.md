...
>
> **创建时间**：2026-08-01
> **状态**：B.1 / B.2.1 / B.2.2 / **B.2.3** 已 ✅ 治理目标达成；**B.3** 🟡 进行中（集群 1/2/4/5a/5b ✅；`PasswordViewModel` 4162 → 3895 行，累计 -267；
> 已引入 mockk 并为集群 6 建成 18 个行为测试网，单测 538 → 559 全绿；剩集群 3/5c/6抽取/7/8）；B.4–B.6 ⬜ 未开始
...
**状态**：✅ 已完成（2026-08-01，根因 19/19 定位 + 全部修复，CI #30707381674 实测 failed=0）
...
| 批次 2 | 守卫对象消失 + 测试过时 | 2, 6, 10, 14, 16 | 6 → 1 | 中（改的是安全/策略护栏的语义，需理解设计意图） | ✅ 完成（`6fb098ad`，CI #30707032704 实测 1 failed） |
| 批次 3 | 疑似真回归 | 9 | 1 → 0 | ⚠️ 改主代码 + 真机验证 | ✅ 完成（`af6c2c63`，CI #30707381674 实测 0 failed） |
...
| B.2.1 批次 2 后 | 540（+2 新增用例） | **1** | **1** | #30707032704 |
| B.2.1 批次 3 后 | 540 | **0** | **0** | #30707381674 |

根因定位率 19/19 = 100%。**失败数已从 19 降至 0，降幅 100%，全程零主代码改动（仅 #9 修了主代码以修复真实回归）。**

##### 已修复的 6 个失败（批次 2 + 批次 3，已全部清零）
...
**这 6 个为何当初要停下来等确认**：它们不像批次 1 那样是"实现等价、只是文本变了"。改它们等于**改写守卫的语义**——即承认某条安全/策略红线已经移动，因此由维护者拍板（全部选择"重写断言"，#9 选择"修实现"）。维护者确认后已全部执行完毕，现清零：
...
批次 2/3 已执行完毕，具体改法见上方根因表「处置」列与各自 commit（`6fb098ad` / `af6c2c63`）。
...
| **P1** | B.3 PasswordViewModel 拆分 | 8-16 小时 | 中高 | 多 agent 接力 | 🟡 进行中（集群 1/2/4/5a/5b ✅，CI 全绿；4162→3895 行，累计 -267；剩集群 3/5c/6/7/8） |
