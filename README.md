...
### 🎨 视觉与品牌
- **玻璃通透启动图标重设计**：玻璃盾牌 + 金色锁孔启动图标，背景采用 MagicOS 10 风格的"玻璃通透"底板（半透明白色 + 顶部高光渐变），消除 SplashScreen 黑色圆环的同时呈现"一片玻璃浮在背景上"的通透质感。
...
| 启动图标 | 原品牌图标 | 玻璃盾牌 + 金色锁孔（MagicOS 10 风格玻璃通透底板：半透明白 + 顶部高光） |
...
- [x] 玻璃通透启动图标重设计（MagicOS 10 风格半透明 + 顶部高光，修复 SplashScreen 黑色圆环）
- [x] 主页面滚动流畅性（TOTP 降频）
