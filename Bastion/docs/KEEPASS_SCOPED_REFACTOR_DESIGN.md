# Bastion Android KeePass 管理域定向重构设计

## 1. 目标与边界

本设计只处理 KeePass 数据库管理与 KeePass 工作区体验，不改变 Bitwarden 与 Bastion 本地数据库的现有行为、同步语义、存储模型与业务流程。

明确边界如下：

- 不修改 Bitwarden 现有同步链路、待同步队列、Cipher 映射与 tombstone 语义。
- 不修改 Bastion 本地密码库、笔记、TOTP、卡片、证件等本地数据表的业务含义。
- 不把 KeePass 改造扩展成三端统一存储层。
- 本轮不引入 Bastion 本地数据库 schema 变更作为前置条件。
...
现状与路线图冲突。路线图已经要求 KeePass 变为独立客户端模式，并停止与 Bastion 本地库、Bitwarden 的双写联动，但代码仍在多个业务 ViewModel 中直接写 KeePass。

现有实现的主要问题：

1. KeePass 不是独立域，而是挂在 Password/Note/Totp/BankCard/Document 等 ViewModel 上的“附属写入目标”。
2. 条目定位依赖 BastionLocalId 或标题/用户名/网址等回退匹配，缺少稳定的原生节点身份。
...
- KeePass 改为独立客户端模式。
- 不再与 Bastion 本地库、Bitwarden 做互通写入。
...
当前 KeePassKdbxService 在更新、删除、恢复时优先使用 BastionLocalId 或 BastionItemId；如果找不到，再回退到字段匹配：

- 密码：Title + UserName + URL
- 安全项：Title + BastionItemType
...
在用户要求“不动 Bitwarden 和 Bastion 本地库”的前提下，本轮设计遵守以下原则：

1. KeePass 只做独立域收口，不做全局存储统一化。
2. Bastion 本地库与 Bitwarden 仍维持原有业务流，不以 KeePass 改造为代价重构。
3. KeePass 工作区中的增删改查，统一以 KDBX 文件内容为唯一事实源。
4. 节点操作优先基于 UUID/节点引用，路径仅用于展示与兼容。
5. 原有 keepassDatabaseId、keepassGroupPath、BastionLocalId 等字段暂时只作为兼容桥，不再继续扩散为未来主模型。
...
3. KeePassCompatibilityBridge
   - 只用于兼容现有字段和旧入口。
   - 作用是把旧的 keepassDatabaseId / keepassGroupPath / BastionLocalId 解释成 KeePass 节点引用。
   - 这是过渡层，不应承载新的业务逻辑。
...
由于本轮不把 Bastion 本地数据库 schema 变更作为前提，建议先采用“运行时原生节点身份 + 兼容字段桥接”的策略。
...
这个引用只存在于 KeePass 域的内存模型和 UI 状态中，不要求落库到 Bastion 本地表。
...
- BastionLocalId / BastionItemId：仅用于旧条目第一次建桥，成功识别 UUID 后不再参与日常主流程。
...
2. 如果当前 UI 来源于旧业务对象，则用 BastionLocalId/BastionItemId 做一次兼容匹配。
...
目标：先把 KeePass 从分散调用改成集中入口，同时不改变 Bitwarden 和 Bastion 本地库行为。
...
- 不改 Bastion 本地密码、笔记、TOTP、卡片、证件的 Room 表语义。
- 不把 KeePass 条目强行并入 Bastion 本地统一列表模型。
- 不以“统一三套存储”为目标启动大重构。
