# Bastion Android KeePass 独立客户端化改造路线图

## 1. 背景与最终决策

### 1.1 决策
- KeePass WebDAV 功能彻底下线（代码、UI、配置、文案、迁移路径全部处理）。
- KeePass 在 Bastion 中改为“独立客户端模式”：
  - 行为目标对齐 KeePassDX（作为独立 KeePass 客户端使用）。
  - 不再与 Bastion 本地库、Bitwarden 做数据互通/双向映射/联动写入。
- 保留 Bastion 本地与 Bitwarden 功能，但与 KeePass 彻底解耦为并行能力。

### 1.2 改造目标
- 在 Bastion 内实现稳定的 KeePass 文件管理体验（打开、浏览、编辑、保存、导入导出、错误提示）。
...
- `Bastion Local Domain`：保持原有本地库逻辑，不感知 KeePass 细节。
- `Bitwarden Domain`：保持独立同步逻辑，不与 KeePass 互相写入。
- `UI 层`：通过“数据源模式/工作区模式”切换，分别进入 KeePass 或 Bastion/Bitwarden 视图，不混合列表。
...
- 移除 KeePass 与 Bastion/Bitwarden 的互通：
...
### 6.2 数据一致性
- KeePass 操作不再影响 Bastion 本地/Bitwarden 数据。
- Bastion 本地/Bitwarden 操作不再触发 KeePass 写入。
...
- 硬件密钥挑战响应（YubiKey HMAC-SHA1 / FIDO2 hmac-secret）：Bastion 尚未接入。
