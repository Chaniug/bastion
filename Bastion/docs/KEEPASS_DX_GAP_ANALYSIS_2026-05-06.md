# Bastion Android KeePass 链路与 KeePassDX 差距分析
...
从当前代码看，这两个现象不是单个页面卡顿点，而是 Bastion KeePass 链路的结构性结果：Bastion 仍然把 KeePass 当成 Bastion 密码体系的一组外部数据源来管理，而 KeePassDX 是 KDBX-first 的独立客户端模型。

最关键的差距是：

1. Bastion 数据库管理页会在数据库列表出现后主动验证所有库，验证会解密打开 KDBX；多个库时会排队。
2. Bastion 底层为了稳定性使用全局单线程 decode 和全局 mutation mutex，多库场景下所有库共享瓶颈。
3. Bastion 的工作区加载把密码、SecureItem、分组拆成多次读取/遍历，缺少“一次打开，一次构建快照”的统一模型。
4. Bastion 仍保留大量 Room 投影和兼容桥逻辑，KeePass 条目的真实身份、父子关系、回收站、历史等没有完全成为业务主模型。
5. KeePassDX 用绑定服务持有单个活动数据库，并用最近文件历史展示轻量元信息；Bastion 则倾向于同时管理和检查多个数据库。

## 2. Bastion 当前 KeePass 链路
...
为了规避 kotpass decode 的稳定性问题，Bastion 做了强串行：
...
即使第一次读取后缓存变热，逻辑层仍然是多次扫描和多次模型转换。对于大库或包含多类型 Bastion 自定义字段的库，加载成本容易放大。
...
Bastion 已经开始补 `keepassEntryUuid`、`keepassGroupUuid`，但整体仍被 PasswordEntry / SecureItem 等 Room 模型牵引。
...
Bastion 的缓存存在于 `KeePassKdbxService` 实例内，而 KeePassDX 是服务级单活动数据库。Bastion 多入口、多 ViewModel 的兼容桥会让缓存收益被稀释。
...
Bastion 当前数据类里已有 KeePass UUID 字段，但主业务仍以 `PasswordEntry`、`SecureItem`、`PasskeyEntry` 等 Bastion 模型为中心。`KeePassCompatibilityBridge` 的方法名里也保留 `Legacy`，说明这仍是过渡层。
...
KeePassDX 的加载任务能报告 key 派生、解密等阶段。Bastion 目前更多是 `OperationState.Loading("...")` 级别，用户不知道卡在哪里。
...
Bastion 的慢点主要来自“列表页主动打开所有库”和“全局串行化 KDBX decode/write”。KeePassDX 的优势不是某个算法更快，而是产品链路更轻：最近文件列表不解密，打开一个库才进入有进度的任务服务，服务持有活动数据库，节点操作围绕内存中的 KDBX 树执行。

因此，Bastion 优先不应该先做 UI 微调，而应该先把“什么时候解密、谁持有打开态、多个库如何排队、远端同步是否阻塞保存”这四件事重新收口。
