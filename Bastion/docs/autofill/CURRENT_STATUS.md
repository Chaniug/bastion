...
`BastionAutofillService` 当前直接创建多个 Dataset 并显示在系统弹窗中,而没有使用我们新开发的 `AutofillPickerActivity`。
...
需要修改 `BastionAutofillService` 或 `SafeResponseBuilder`,在有多个密码匹配时:
...
**选项2: 修改 BastionAutofillService.kt**
...
用户触发自动填充
    ↓
BastionAutofillService.onFillRequest()
    ↓
创建多个 Dataset (每个密码一个)
    ↓
系统显示原生列表 ← 用户看到的就是这个
...
1. 找到 `BastionAutofillService` 中创建 FillResponse 的具体位置
2. 添加密码数量判断逻辑
...
- `app/src/main/java/com/bastion/app/autofill/core/SafeResponseBuilder.kt`
- `app/src/main/java/com/bastion/app/autofill/AutofillPickerLauncher.kt`
- `app/src/main/java/com/bastion/app/autofill/AutofillPickerActivity.kt`
