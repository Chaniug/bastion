...
本文档提供了集成和测试 Bastion 自动填充功能的完整检查清单，确保所有组件正确集成并正常工作。
...
- [x] 在 `BastionAutofillService.onCreate()` 中初始化
...
# 启动应用
adb shell am start -n com.bastion.app/.MainActivity
```
...
```bash
# 查看自动填充日志
adb logcat | grep BastionAutofill

# 查看诊断日志
adb logcat | grep AutofillDiagnostics

# 查看错误日志
adb logcat *:E | grep Bastion
```
