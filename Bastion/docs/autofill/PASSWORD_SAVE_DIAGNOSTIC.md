...
- [ ] Bastion 应用已安装
- [ ] 已在系统设置中启用 Bastion 的自动填充服务
  - **路径**: 设置 → 系统 → 语言和输入法 → 自动填充服务 → 选择 Bastion
- [ ] Bastion 应用有必要的权限
...
**步骤 2: 显示保存界面**
- [ ] **预期**: 应该弹出 Bastion 的密码保存对话框（BottomSheet）
- [ ] 对话框显示正确的用户名和密码
- [ ] 可以编辑标题、网站等信息
...
1. 检查自动填充服务设置
   ...
   ```
   应该显示: `com.bastion.app/.autofill.BastionAutofillService`

2. 如果未设置，手动启用:
   - 设置 → 系统 → 语言和输入法 → 自动填充服务 → Bastion
...
2. 检查数据库权限:
   ```powershell
   adb shell run-as com.bastion.app ls -la /data/data/com.bastion.app/databases/
   ```

3. 检查加密服务是否正常工作

### 问题 4: 保存成功，但在 Bastion 中看不到
...
```powershell
# 只看 Bastion 的日志
adb logcat -v time | Select-String "BastionAutofill|AutofillSave"
...
# 清除日志后重新测试
adb logcat -c
adb logcat -v time | Select-String "BastionAutofill"
```
...
# 启用 Bastion 自动填充（如果未启用）
adb shell settings put secure autofill_service com.bastion.app/.autofill.BastionAutofillService
