...
| Agent（重点项） | 方案 B：Autofill 配置预加载缓存（新增 `AutofillConfigCache` 进程单例 + `BastionAutofillServiceNg.onCreate` 预加载 + 观测 `settingsFlow`/`isInlineSuggestionsEnabled`/`isAutofillEnabled` 持续刷新；AccountFillPolicy / FilledDataBuilderNg / AutofillPickerActivityV2 / `onFillRequest` 关键读改读缓存，消除填充热路径 `runBlocking`） | `5c0fdbfa` → 合并 main `5c0fdbfa` | #31045278168(dev) **Success** ✅ → #31045863696(main) **Success** ✅ | 已完成并通过 CI；preview APK 已发布（build.202608052054） |
