...
import com.bastion.app.passkey.PasskeyPrivateKeySupport

/**
 * Passkey 数据映射器
 * 
 * BastionkeyEntry <-> Bitwarden Login (Type 1)
 * 
 * 同步策略：
 * - Bastion → Bitwarden: 优先同步完整 passkey（包含私钥材料）
 * - Bitwarden → Bastion: 支持导入可用 passkey；缺少私钥时降级为引用记录
 */
class PasskeyMapper : BitwardenMapper<PasskeyEntry> {
    ...
    private fun buildPasskeyNotes(item: PasskeyEntry): String {
        ...
        return buildString {
            ...
            appendLine()
            appendLine("🔐 This is a Passkey entry synced from Bastion")
            appendLine("ℹ️ Private key availability depends on client capability.")
            appendLine()
            appendLine("---")
            appendLine("[Bastionkey Metadata]")
    ...
    private fun parsePasskeyMetadata(notes: String?): PasskeyMetadata? {
        if (notes == null || !notes.contains("[Bastionkey Metadata]")) return null
        
        try {
            val lines = notes.lines()
            val dataLines = lines.dropWhile { it != "[Bastionkey Metadata]" }.drop(1)
    ...
    companion object {
        ...
        fun isPasskeyCipher(cipher: CipherApiResponse): Boolean {
            ...
            return cipher.name?.endsWith(" [Passkey]") == true ||
                cipher.notes?.contains("[Bastionkey Metadata]") == true
        }
