 ...
 * 原始字节与 Bastion 的 [Attachment] / [AttachmentStorage] 密文格式互转：
 *
 * - `upload`：原始字节 → kdbx binary pool，同时在 Bastion 本地 Local_Encrypted_Store
 *   再写一份缓存密文（便于后续 offline 预览/导出）。
 * - `download`：对已经在 kdbx pool 里的 binary，解出字节 → 再以 Bastion 本地格式缓存。
...
class KeePassAttachmentExecutor(
    ...
    /**
     * 把明文字节写入 KeePass 数据库，并在 Bastion 本地缓存一份 GCM 密文副本。
     *
     * 失败时：
    ...
    /**
     * 把已经在 kdbx 池里的某个附件字节"物化"到 Bastion 本地密文缓存，并返回更新后的 [Attachment]。
     *
     * 适用场景：
