...
import com.bastion.app.data.model.TIMELINE_FIELD_MAINTENANCE_SNAPSHOT_PAYLOAD
import com.bastion.app.data.model.TimelineBatchCopyPayload
import com.bastion.app.data.model.TimelineMaintenanceSnapshotPayload
...
import com.bastion.app.repository.OperationLogRepository
...
class TimelineViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun convertLogsToEvents(
        ...
        snapshotsByLogId: Map<Long, com.bastion.app.data.TimelineVersionSnapshot> = emptyMap()
    ): List<TimelineEvent> {
        return logs.map { log ->
            ...
            "PASSWORD" -> {
                ...
                changes.forEach { change ->
                    val current = when (change.fieldName) {
                        "用户名" -> entry.username
                        "网站" -> entry.website
                        "密码" -> securityManager.decryptDataIfBastionCiphertext(entry.password)
                        "备注" -> entry.notes
                        "标题" -> entry.title
                        else -> return@forEach
                    }
            ...
            "TOTP", "BANK_CARD", "NOTE", "DOCUMENT", "BILLING_ADDRESS" -> {
                ...
                changes.forEach { change ->
                    val current = when (change.fieldName) {
                        "标题" -> item.title
                        "备注" -> item.notes
                        com.bastion.app.data.TIMELINE_SNAPSHOT_FIELD_ITEM_DATA -> item.itemData
                        com.bastion.app.data.TIMELINE_SNAPSHOT_FIELD_NOTES -> item.notes
                        else -> return@forEach
                    }
                    if (current != expectedValue(change)) return false
                    updated = when (change.fieldName) {
                        "标题" -> updated.copy(title = targetValue(change))
                        "备注" -> updated.copy(notes = targetValue(change))
                        com.bastion.app.data.TIMELINE_SNAPSHOT_FIELD_ITEM_DATA ->
                            updated.copy(itemData = targetValue(change))
                        com.bastion.app.data.TIMELINE_SNAPSHOT_FIELD_NOTES ->
    ...
    private suspend fun restoreBatchMoveRemoteState(
        entry: com.bastion.app.data.PasswordEntry,
        targetState: com.bastion.app.data.model.TimelinePasswordLocationState
    ): Boolean {
