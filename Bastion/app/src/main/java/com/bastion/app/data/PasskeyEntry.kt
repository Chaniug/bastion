...
data class PasskeyEntry(
    ...
    // Passkey 模式:
    // LEGACY         -> 旧 Bastion 通行密钥（保留本地兼容，不参与 Bitwarden 可用同步）
    // BW_COMPAT      -> Bitwarden 兼容模式（可参与 Bitwarden/Keyguard 同步）
    // KEEPASS_COMPAT -> KeePassDX/KeePassXC KPEX_PASSKEY_* 兼容格式（可与 Bastion 写入/回读的 KDBX 互通）
    ...
    /** Bastion 展示名称：用户备注优先，不改写 WebAuthn 原始名称。 */
