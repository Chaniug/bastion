...
import com.bastion.app.data.model.TIMELINE_FIELD_BATCH_MOVE_PAYLOAD
import com.bastion.app.data.model.TimelineBatchCopyPayload
import com.bastion.app.data.model.TimelineBatchMovePayload
import com.bastion.app.data.model.TimelinePasswordLocationState
import com.bastion.app.data.model.TimelinePasswordRecreatedEntry
...
internal data class PasswordBatchMoveTargetRouting(
    ...
    val bastionCategoryId: Long?,
    val isBastionCopyTarget: Boolean
)
...
internal fun resolvePasswordBatchMoveTargetRouting(
    ...
    val isArchiveTarget = target is UnifiedMoveCategoryTarget.BastionCategory &&
        target.categoryId == UNIFIED_MOVE_ARCHIVE_SENTINEL_CATEGORY_ID
    val bastionCategoryId = when (target) {
        UnifiedMoveCategoryTarget.Uncategorized -> null
        is UnifiedMoveCategoryTarget.BastionCategory ->
            target.categoryId.takeUnless { it == UNIFIED_MOVE_ARCHIVE_SENTINEL_CATEGORY_ID }
        else -> null
    }
    val isBastionCopyTarget = target == UnifiedMoveCategoryTarget.Uncategorized ||
        (target is UnifiedMoveCategoryTarget.BastionCategory && !isArchiveTarget)
    return PasswordBatchMoveTargetRouting(
        isArchiveTarget = isArchiveTarget,
        bastionCategoryId = bastionCategoryId,
        isBastionCopyTarget = isBastionCopyTarget
    )
}
...
internal fun toMovedLocationState(
    ...
    val archivedAt = if (target is UnifiedMoveCategoryTarget.BastionCategory &&
        target.categoryId == UNIFIED_MOVE_ARCHIVE_SENTINEL_CATEGORY_ID
    ) {
        ...
        is UnifiedMoveCategoryTarget.BastionCategory -> {
            if (target.categoryId == UNIFIED_MOVE_ARCHIVE_SENTINEL_CATEGORY_ID) {
                ...
                categories.find { it.id == target.categoryId }?.name
                    ?: context.getString(R.string.filter_bastion)
            }
        }
...
internal suspend fun executePasswordBatchCopy(
    ...
    if (targetRouting.isBastionCopyTarget) {
        selectedEntries.forEach { entry ->
            val createdId = copyPasswordToBastionLocal(entry, targetRouting.bastionCategoryId)
    ...
    // 复制源密码的本地附件到新密码（仅 Bastion-local 目标）。
    // 对 Bitwarden / KeePass 目标不做复制：前者服务端不兼容 free 账户附件；
    // 后者由各自 executor 在 kdbx 落地流程中自行处理。
    if (targetRouting.isBastionCopyTarget && idPairs.isNotEmpty()) {
        val facade = com.bastion.app.attachments.AttachmentContainer.facade(context)
        idPairs.forEach { (sourceId, newId) ->
            runCatchingObserved { facade.cloneAttachmentsToNewParent(sourceId, newId) }
                .onFailure { e ->
                    android.util.Log.w(
                        "PasswordBatchCopy",
...
internal fun PasswordBatchMoveSheet(
        ...
        onTargetSelected = { target, action ->
            ...
            viewModel.viewModelScope.launch {
                ...
                if (bitwardenMoveTargetVaultId != null) {
                    val vaultIsPremium = com.bastion.app.bitwarden.BitwardenVaultPremiumStore
                        .isPremium(context, bitwardenMoveTargetVaultId)
                    val advisor = com.bastion.app.attachments.AttachmentContainer
                            ...
                            when (target) {
                                ...
                                is UnifiedMoveCategoryTarget.KeePassGroupTarget -> {
                                    ...
                                    val copyResult = executePasswordBatchCopy(
                                        ...
                                        copyPasswordToBastionLocal = { entry, categoryId ->
                                            viewModel.copyPasswordToBastionLocal(
                                                entry = entry,
                            ...
                            when {
                                ...
                                target == UnifiedMoveCategoryTarget.Uncategorized -> {
                                    ...
                                    if (keepassEntries.isNotEmpty()) {
                                        ...
                                        val result = viewModel.moveKeePassPasswordsToBastionCategoryAwait(
                                            ids = keepassIds,
                                            categoryId = null
                                        )
                                        if (result.isFailure) {
                                    ...
                                    bitwardenEntries.forEach { entry ->
                                        val result = viewModel.moveBitwardenPasswordToBastionLocal(entry, null)
                                        if (result.isFailure) {
                                            throw result.exceptionOrNull()
                                ...
                                target is UnifiedMoveCategoryTarget.BastionCategory -> {
                                    ...
                                    bitwardenEntries.forEach { entry ->
                                        val result = viewModel.moveBitwardenPasswordToBastionLocal(
                                            entry = entry,
                                            categoryId = target.categoryId
                                        )
                                        if (result.isFailure) {
    ...
    attachmentAwarePrompt?.let { prompt ->
        com.bastion.app.attachments.ui.AttachmentAwareMoveDialog(
...
private data class AttachmentAwareMovePrompt(
    val classification: com.bastion.app.attachments.facade.AttachmentBatchMoveAdvisor.Classification,
