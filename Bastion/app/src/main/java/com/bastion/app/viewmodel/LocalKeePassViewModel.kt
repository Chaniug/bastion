...
import com.bastion.app.data.KeePassOpenMode
import com.bastion.app.data.KeePassRemoteProviderType
...
import com.bastion.app.data.KeepassRemoteSyncState
...
import com.bastion.app.sync.KEEPASS_REMOTE_SYNC_DEDUPE_KEY
import com.bastion.app.sync.SyncDiagnostics
import com.bastion.app.sync.SyncEnqueueResult
...
import com.bastion.app.utils.GoogleDriveKeePassFileSource
import com.bastion.app.utils.GoogleDriveKeePassSupport
...
import com.bastion.app.utils.OneDriveKeePassSupport
import com.bastion.app.utils.OperationLogger
import com.bastion.app.utils.RemoteKeePassSyncService
import com.bastion.app.utils.WebDavKeePassFileSource
import com.bastion.app.utils.WebDavKeePassSupport
import java.io.File
...
class LocalKeePassViewModel(
    ...
    suspend fun movePasswordEntriesToBastionLocal(
        entries: List<PasswordEntry>
    ): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            // 但我们在 Bastion 侧为每个 KEEPASS 附件保留了本地 GCM 密文缓存 + wrappedCek，
            ...
            if (entryUuid.isNullOrBlank()) {
                val hasKeePassAttachments = repository
                    .listByParentAndSource(
                        passwordId = entry.id,
                        source = com.bastion.app.attachments.model.AttachmentSource.KEEPASS
                    )
                    .isNotEmpty()
    ...
    private fun createEmptyKdbxContent(
        ...
        val meta = Meta(
            generator = "Bastionword Manager",
            name = databaseName.ifBlank { "Bastion Database" }
        )
