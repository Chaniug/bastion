...
sealed class SecureItemOwnership {
    object BastionLocal : SecureItemOwnership()
...
fun SecureItem.resolveOwnership(): SecureItemOwnership {
    ...
    return when {
        hasKeePassBinding && hasBitwardenBinding -> when {
            ...
            !hasConcreteKeePassBinding && !hasConcreteBitwardenBinding -> SecureItemOwnership.BastionLocal
        ...
        hasBitwardenBinding -> if (hasConcreteBitwardenBinding) {
            ...
            SecureItemOwnership.BastionLocal
        }
        else -> SecureItemOwnership.BastionLocal
    }
}
...
fun SecureItem.asBastionLocalCopy(categoryId: Long?): SecureItem {
    return copy(
        id = 0,
...
fun SecureItem.isLocalOnlyItem(): Boolean = resolveOwnership() is SecureItemOwnership.BastionLocal
