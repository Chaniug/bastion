...
import com.bastion.app.utils.KeePassCodecSupport
import com.bastion.app.utils.KeePassCredentialSupport
import com.bastion.app.utils.KeePassEntryResolutionContext
import com.bastion.app.utils.KeePassErrorCode
import com.bastion.app.utils.KeePassFieldReferenceResolver
import com.bastion.app.utils.KeePassOperationException
import com.bastion.app.utils.KeePassFormatInspector
...
class KeePassKdbxViewModel {
        ...
        val totpEntries = totpItems.mapNotNull { item ->
            try {
                ...
                val decryptedSecret = try {
                    securityManager.decryptDataIfBastionCiphertext(totpData.secret)
                } catch (e: Exception) {
        ...
        try {
            ...
            allEntries.forEach { (entry, groupPath) ->
                try {
                    ...
                    val bastionItemType = getFieldValue("BastionItemType")

                    // Bastion 安全项映射条目不应按密码导入。
                    if (bastionItemType.isNotBlank()) {
                        return@forEach
                    }
                    ...
                    if (shouldImportPassword) {
                        ...
                        val insertedPasswordId = if (existingEntry != null) {
                            ...
                            val passwordEntry = com.bastion.app.data.PasswordEntry(
                                title = title,
                                username = username,
                                password = encryptedPassword,
                                website = url,
                        ...
                        if (insertedPasswordId > 0) {
                            ...
                            val standardFields = setOf(
                                ...
                                "otp", "TOTP Seed", "TOTP Settings", "BastionItemType",
                                ...
                                "SSID", "BastionLoginType", "BastionWifiData",
    ...
    private fun resolveSshKeyData(
        ...
        return SshKeyDataCodec.encode(
            SshKeyData(
                algorithm = field("BastionSshAlgorithm"),
                keySize = field("BastionSshKeySize").toIntOrNull() ?: 0,
                publicKeyOpenSsh = field("BastionSshPublicKey"),
                privateKeyOpenSsh = field("BastionSshPrivateKey"),
                fingerprintSha256 = field("BastionSshFingerprint"),
                comment = field("BastionSshComment"),
                format = field("BastionSshFormat").ifBlank { SshKeyData.FORMAT_OPENSSH }
            )
        )
    }
     ...
     *
     * 优先级：
     *  1. Bastion 自己写入的 `BastionLoginType + BastionWifiData`（完整保真）
     *  2. `BastionLoginType=WIFI` 但没有 JSON 时，基于 SSID 构造一个最小 WifiData
    ...
    private fun resolveWifiMetadata(
        ...
        val bastionLoginType = field("BastionLoginType")
        val bastionWifiJson = field("BastionWifiData")
        ...
        return when {
            ...
            bastionLoginType.equals("WIFI", ignoreCase = true) -> {
                val wifi = com.bastion.app.data.model.WifiData(
            ...
            bastionLoginType.equals("SSO", ignoreCase = true) -> "SSO" to ""
    ...
    private fun isImportablePasswordEntry(
        ...
        // 带 Bastion 安全项标记的条目不是密码项（避免计数虚高）。
        if (field("BastionItemType").isNotBlank()) return false
    ...
    private fun parseOtpAuthUri(
        uri: String,
        securityManager: com.bastion.app.security.SecurityManager
    ): TotpData? {
        try {
