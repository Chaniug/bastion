...
import com.bastion.app.keepass.KeePassSecureItemCreateExecutor
import com.bastion.app.keepass.KeePassSecureItemDeleteExecutor
import com.bastion.app.keepass.KeePassSecureItemUpdateExecutor
import com.bastion.app.keepass.KeePassTotpProjectionMatcher
import com.bastion.app.bitwarden.BitwardenHistoricalRepairStateHelper
import com.bastion.app.bitwarden.SecureItemBitwardenTransitionResolver
...
import com.bastion.app.data.hasBitwardenBinding
import com.bastion.app.data.hasOwnershipConflict
...
class TotpViewModel(
    ...
    private fun decryptStoredSensitiveValue(value: String): String {
        return securityManager?.decryptDataIfBastionCiphertext(value) ?: value
    }

    private fun looksLikeStoredSensitiveCiphertext(value: String): Boolean {
        return securityManager?.looksLikeBastionCiphertext(value) == true
    }
    ...
    private fun StorageTarget.toTotpCategoryFilter(): TotpCategoryFilter {
        return when (this) {
            is StorageTarget.BastionLocal -> categoryId
    ...
    fun saveTotpItem(
        ...
        viewModelScope.launch {
            try {
                ...
                Log.e(
                    "TotpViewModel",
                    "saveTotpItem failed id=$id categoryId=$categoryId keepassDatabaseId=$keepassDatabaseId bitwardenVaultId=$bitwardenVaultId error=${e::class.java.simpleName}: ${e.message}",
                    e
                )
            }
        }
                ...
                if (distinctTargets.isEmpty()) {
                    ...
                    suspend fun saveIntoTarget(target: StorageTarget, targetId: Long?): Boolean {
                        val savedId = when (target) {
                            is StorageTarget.BastionLocal -> saveTotpItemInternal(
                                id = targetId,
                                title = title,
                                notes = notes,
                                totpData = totpData,
    ...
    private suspend fun saveTotpItemInternal(
        ...
        val savedItemId = if (id != null && id > 0) {
            ...
            val newId = keepassSecureItemCreateExecutor.create(
                ...
                Log.e(
                    "TotpViewModel",
                    "saveTotpItemInternal failed while creating item keepassDatabaseId=$resolvedKeepassDatabaseId bitwardenVaultId=$resolvedBitwardenVaultId"
                )
                return null
            }
            requestBitwardenMutationSync(resolvedBitwardenVaultId)
    ...
    fun incrementHotpCounter(itemId: Long) {
        viewModelScope.launch {
            try {
                ...
                // 只处理HOTP类型
                if (totpData.otpType != com.bastion.app.data.model.OtpType.HOTP) {
                    return@launch
                }
    ...
    suspend fun copyTotpToBastionLocal(
        item: SecureItem,
        categoryId: Long?
    ): Long? {
        ...
        val localCopy = item.asBastionLocalCopy(categoryId).copy(
            itemData = encodeStoredSensitiveValueForRewrite(
                item.itemData,
    ...
    suspend fun moveTotpToBastionLocal(
        item: SecureItem,
        categoryId: Long?
    ): Result<Long> {
        ...
        if (item.hasOwnershipConflict()) {
            return Result.failure(IllegalStateException("验证器来源冲突，无法移动到 Bastion 本地"))
        }

        val newId = copyTotpToBastionLocal(item, categoryId)
            ?: return Result.failure(IllegalStateException("创建 Bastion 本地验证器副本失败"))
        ...
        val sourceDelete = when (val ownership = item.resolveOwnership()) {
            ...
            is SecureItemOwnership.Conflict -> Result.failure(IllegalStateException("验证器来源冲突，无法移动到 Bastion 本地"))
        }

        if (sourceDelete.isFailure) {
            Log.e(
                "TotpViewModel",
                "TOTP move to Bastion local kept target copy after source cleanup failed; sourceId=${item.id} targetId=$newId error=${sourceDelete.exceptionOrNull()?.message}"
            )
    ...
    suspend fun moveTotpToStorage(
        ...
        return try {
            ...
            if (keepassSync.isFailure) {
                Log.e(
                    "TotpViewModel",
                    "TOTP move failed before local update; sourceId=$id keepassTarget=$keepassDatabaseId bitwardenTarget=$bitwardenVaultId error=${keepassSync.exceptionOrNull()?.message}"
                )
                return false
            }
