...
sealed interface UnifiedMoveCategoryTarget {
    ...
    data class BastionCategory(val categoryId: Long) : UnifiedMoveCategoryTarget
...
private sealed interface MovePickerSource {
    ...
    data object BastionLocal : MovePickerSource {
...
fun UnifiedMoveToCategoryBottomSheet(
    ...
    val activeSourceKey = remember { mutableStateOf(sources.firstOrNull()?.key ?: MovePickerSource.BastionLocal.key) }
    val sourceKeys = sources.map { it.key }
    LaunchedEffect(sourceKeys) {
        if (activeSourceKey.value !in sourceKeys) {
            activeSourceKey.value = MovePickerSource.BastionLocal.key
    ...
    val bastionCategoryNodes = remember(categories) { buildBastionCategoryNodes(categories) }
    ...
    val bastionLabel = stringResource(R.string.filter_bastion)
    ...
    fun supportingLabelForSource(source: MovePickerSource): String? {
        return when (source) {
            MovePickerSource.BastionLocal -> null
    ...
    fun stageRootTargetForSource(source: MovePickerSource) {
        when (source) {
            MovePickerSource.BastionLocal -> stageTarget(
                target = UnifiedMoveCategoryTarget.Uncategorized,
                label = "$bastionLabel / $categoryNoneLabel"
            )
    ...
    fun buildTargetsForSource(source: MovePickerSource): List<MovePickerTarget> {
        return when (source) {
            MovePickerSource.BastionLocal -> buildList {
                if (allowArchiveTarget) {
                    add(
                        MovePickerTarget(
                            target = UnifiedMoveCategoryTarget.BastionCategory(UNIFIED_MOVE_ARCHIVE_SENTINEL_CATEGORY_ID),
                            label = "$bastionLabel / $archiveLabel",
                            title = archiveLabel,
                ...
                bastionCategoryNodes.forEach { node ->
                    add(
                        MovePickerTarget(
                            target = UnifiedMoveCategoryTarget.BastionCategory(node.category.id),
                            label = listOfNotNull(bastionLabel, node.parentPathLabel, node.displayName)
                                .joinToString(" / "),
        ...
        Column(
            ...
            LazyColumn(
                ...
                if (activeTargets.isEmpty()) {
                    ...
                    items(activeTargets, key = { it }) { target ->
                        MoveTargetItem(
                            title = target.title,
                            icon = target.icon,
...
private data class BastionCategoryNode(
