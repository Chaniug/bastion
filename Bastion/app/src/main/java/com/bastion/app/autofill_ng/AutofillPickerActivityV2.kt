...
import com.bastion.app.autofill_ng.ui.*
import com.bastion.app.autofill_ng.utils.SmartCopyNotificationHelper
import com.bastion.app.data.LocalKeePassDatabase
import com.bastion.app.data.ItemType
import com.bastion.app.data.isLocalPasswordOwnership
...
import com.bastion.app.data.model.displayFullName
...
import com.bastion.app.ui.screens.AddEditPasswordInitialDraft
...
import com.bastion.app.util.PasswordGenerator
 ...
 * - Dropdown 菜单选择操作
 */
class AutofillPickerActivityV2 : BaseBastionActivity() {
    ...
    // attachBaseContext 已由 BaseBastionActivity 统一处理
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            // 读取截图保护设置（截图保护已由 BaseBastionActivity 统一处理）
            val settings by localSettingsManager.settingsFlow.collectAsState(
                initial = com.bastion.app.data.AppSettings()
            )
    ...
    private fun handleBankCardAutofill(item: SecureItem) {
        val (_, data) = parseBankCardCandidate(
            item,
            decryptIfNeeded = SecurityManager(applicationContext)::decryptDataIfBastionCiphertext
        ) ?: run {
            setResult(Activity.RESULT_CANCELED)
            finish()
            return
...
private fun AutofillPickerContent(
    ...
    val secureItemRepository = remember(appDb, securityManager) {
        SecureItemRepository(
            appDb.secureItemDao(),
            decryptSensitiveValue = securityManager::decryptDataIfBastionCiphertext
        )
    }
    ...
    AnimatedContent(
        ...
        "list" -> {
            ...
            val title = when {
                ...
                else -> stringResource(R.string.autofill_with_bastion)
            }
...
private fun AutofillStorageSourceFilter.labelResId(): Int = when (this) {
    ...
    AutofillStorageSourceFilter.LOCAL -> R.string.filter_bastion
