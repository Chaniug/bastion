...
class AutofillPreferences(private val context: Context) {
    ...
    companion object {
        /**
         * 自动填充配置必须使用独立于 "settings" 的 DataStore，切勿合并。
         *
         * 原因：BastionAccessibilityService 运行在独立进程 :accessibility
         * （AndroidManifest 中 android:process=":accessibility"），会订阅本类的
         * isActiveFillNotificationEnabled。而 DataStore 不支持多进程——它只在
         * 本进程写入时 emit 新值，不监听文件变化。
         *
         * 若把这些 key 并入由主进程高频写入的 "settings"，:accessibility 进程
         * （开机由系统自启，可能先于主进程）会读到尚未写入的快照并永久缓存，
         * 导致「主动填充通知」静默失效，直到无障碍服务重启。
         */
