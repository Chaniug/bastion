...
class DocumentViewModel(
        ...
        viewModelScope.launch {
            ...
            distinctTargets
                ...
                .forEach { target ->
                    ...
                    when (target) {
                        is StorageTarget.BastionLocal -> if (existingReplica == null) addDocument(
                            title = title,
                            documentData = documentData,
                            notes = notes,
                            isFavorite = isFavorite,
    ...
    suspend fun copyDocumentToBastionLocal(
        item: SecureItem,
        categoryId: Long?
    ): Long? {
    ...
    suspend fun moveDocumentToBastionLocal(
        item: SecureItem,
        categoryId: Long?
    ): Result<Long> {
        ...
        if (item.hasOwnershipConflict()) {
            return Result.failure(IllegalStateException("证件来源冲突，无法移动到 Bastion 本地"))
        }

        val newId = copyDocumentToBastionLocal(item, categoryId)
            ?: return Result.failure(IllegalStateException("创建 Bastion 本地证件副本失败"))
        ...
        if (sourceDelete.isFailure) {
            Log.e(
                "DocumentViewModel",
                "Document move to Bastion local kept target copy after source cleanup failed; sourceId=${item.id} targetId=$newId error=${sourceDelete.exceptionOrNull()?.message}"
            )
    ...
    private fun emptyDocumentData() = DocumentData(
        documentNumber = "",
        documentType = com.bastion.app.data.model.DocumentType.ID_CARD,
        fullName = "",
        issuedDate = "",
        expiryDate = ""
    )
