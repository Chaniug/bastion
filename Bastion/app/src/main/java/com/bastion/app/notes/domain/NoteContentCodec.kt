...
object NoteContentCodec {
    private const val INLINE_IMAGE_SCHEME = "bastion-image://"
    private val INLINE_IMAGE_REGEX = Regex("!\\[[^\\]]*\\]\\(bastion-image://([^\\)\\s]+)\\)")
    private val INLINE_IMAGE_MARKDOWN_REGEX = Regex("!\\[([^\\]]*)\\]\\(bastion-image://([^\\)\\s]+)\\)")
    ...
    fun removeInlineImageRef(content: String, imageId: String): String {
        ...
        val refRegex = Regex("!\\[[^\\]]*\\]\\(bastion-image://$escapedId\\)\\s*")
        return content
            .replace(refRegex, "")
            .replace(Regex("\\n{3,}"), "\n\n")
            .trimEnd()
