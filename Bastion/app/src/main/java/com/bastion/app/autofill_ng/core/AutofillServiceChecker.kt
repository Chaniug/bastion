...
class AutofillServiceChecker(private val context: Context) {
    ...
    private fun bastionAutofillComponents(): List<ComponentName> {
        return listOf(
            ComponentName(context, BastionAutofillServiceNg::class.java),
        )
    }
    ...
    data class ServiceStatus(
        ...
        fun getSummary(context: Context): String {
            return when {
                isFullyOperational() -> context.getString(com.bastion.app.R.string.autofill_status_summary_operational)
                !isServiceDeclared -> context.getString(com.bastion.app.R.string.autofill_status_summary_not_declared)
                !isSystemEnabled -> context.getString(com.bastion.app.R.string.autofill_status_summary_system_disabled)
                !isAppEnabled -> context.getString(com.bastion.app.R.string.autofill_status_summary_app_disabled)
                !hasRequiredPermissions -> context.getString(com.bastion.app.R.string.autofill_status_summary_no_permissions)
                compatibilityIssues.isNotEmpty() -> context.getString(com.bastion.app.R.string.autofill_status_summary_compatibility_issues)
                else -> context.getString(com.bastion.app.R.string.autofill_status_summary_abnormal)
            }
        }
    }
    ...
    private fun checkServiceDeclared(): Boolean {
        return try {
            ...
            val targetComponents = bastionAutofillComponents()
            ...
            if (!declared) {
                AutofillLogger.e(TAG, "No Bastion autofill services declared with required permission")
            }
            declared
        } catch (e: Exception) {
    ...
    private fun generateRecommendations(
        ...
        try {
            ...
            if (!isServiceDeclared) {
                recommendations.add("请检查 AndroidManifest.xml 中是否正确声明了 BastionAutofillService")
                recommendations.add("确保服务包含 BIND_AUTOFILL_SERVICE 权限和 autofill intent-filter")
            }
            
            // 2. 系统未启用
            if (!isSystemEnabled) {
                recommendations.add("请在系统设置中启用 Bastion 作为自动填充服务")
            ...
            // 3. 应用内未启用
            if (!isAppEnabled) {
                recommendations.add("请在 Bastion 应用设置中启用自动填充功能")
            }
            ...
            if (compatibilityIssues.isNotEmpty()) {
                ...
                compatibilityIssues.forEach { issue ->
                    when {
                        issue.contains("小米") -> {
                            recommendations.add("小米设备：打开安全中心 → 应用管理 → Bastion → 权限管理 → 允许自动填充")
                        }
                        issue.contains("华为") -> {
                            recommendations.add("华为设备：可能需要在手机管家中允许 Bastion 自启动")
                        }
                        issue.contains("OPPO") || issue.contains("Realme") -> {
                            recommendations.add("OPPO/Realme：设置 → 应用管理 → Bastion → 权限 → 允许自动填充")
                        }
                        issue.contains("Vivo") -> {
                            recommendations.add("Vivo：i 管家 → 应用管理 → 权限管理 → Bastion → 允许自动填充")
                        }
                        issue.contains("内联建议") -> {
                            recommendations.add("设备不支持内联建议，将使用传统下拉菜单模式")
                        }
                        issue.contains("Android 12 国产 ROM") -> {
                            recommendations.add("请在系统设置中关闭 Bastion 的省电限制，并允许自启动/后台运行")
