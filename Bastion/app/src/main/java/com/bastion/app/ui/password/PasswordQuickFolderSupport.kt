...
internal fun buildLocalQuickFolderPasswordCountByCategoryId(
    entries: List<com.bastion.app.data.PasswordEntry>,
    categories: List<Category>
): Map<Long, Int> {
    val localCategories = categories.filter(Category::isBastionLocalCategory)
...
internal fun buildQuickFolderShortcuts(
    ...
    searchScopedPasswords: List<com.bastion.app.data.PasswordEntry>,
    ...
    val shouldShowBastionFolderShortcuts = when (currentFilter) {
    ...
    if (shouldShowBastionFolderShortcuts) {
    ...
    return when (quickFolderStyle) {
        com.bastion.app.data.PasswordListQuickFolderStyle.CLASSIC,
        com.bastion.app.data.PasswordListQuickFolderStyle.M3_CARD -> shortcuts
    }
}
...
internal fun buildCategoryMenuFolderShortcuts(
    ...
    val shouldShowBastionFolders = when (currentFilter) {
        is CategoryFilter.All,
        is CategoryFilter.Local,
        is CategoryFilter.Starred,
        is CategoryFilter.Uncategorized,
    ...
    when (val filter = currentFilter) {
        is CategoryFilter.Custom -> {
            if (quickFolderRootFilter is CategoryFilter.All) {
                crumbs += PasswordQuickFolderBreadcrumb("source_bastion", "Bastion", CategoryFilter.Local, quickFolderCurrentPath == null)
            }
...
private fun appendRootDatabaseShortcuts(
    ...
    currentFilter: CategoryFilter,
    quickFolderSourceEntries: List<com.bastion.app.data.PasswordEntry>,
    isSearchActive: Boolean,
    keepassDatabases: List<com.bastion.app.data.LocalKeePassDatabase>,
    bitwardenVaults: List<com.bastion.app.data.bitwarden.BitwardenVault>,
    categories: List<Category>
