...
object SteamMaFileBackupCodec {
    ...
    fun encode(account: SteamAccount): String {
        ...
        if (account.hasRealSteamId) {
            ...
            root["bastion_missing_steamid"] = JsonPrimitive(true)
            putString("bastion_local_steamid", account.steamId)
        }
        putString("account_name", account.accountName.ifBlank { account.displayName })
        root.remove("bastion_display_name")
        ...
        putString("bastion_display_name", remarkName)
    ...
    private fun removeMissingSteamIdMarkers(root: MutableMap<String, kotlinx.serialization.json.JsonElement>) {
        root.remove("bastion_missing_steamid")
        root.remove("bastionMissingSteamId")
        root.remove("bastion_local_steamid")
        root.remove("bastionLocalSteamId")
    }
