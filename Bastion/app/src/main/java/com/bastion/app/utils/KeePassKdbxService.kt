...
import com.bastion.app.data.KeePassDatabaseSourceType
import com.bastion.app.data.KeePassDatabaseCreationOptions
...
import com.bastion.app.data.resolvedActiveFilePath
import com.bastion.app.data.resolvedActiveStorageLocation
...
import com.bastion.app.keepass.KeePassDxPasskeyCodec
import com.bastion.app.keepass.KeePassChangeOperation
import com.bastion.app.keepass.KeePassChangeSet
import com.bastion.app.keepass.KeePassChangeSetApplier
import com.bastion.app.keepass.KeePassChangeTarget
import com.bastion.app.keepass.KeePassAttachmentChangePatch
import com.bastion.app.keepass.KeePassEntryCreatePatch
import com.bastion.app.keepass.KeePassFieldChangePatch
import com.bastion.app.keepass.KeePassAutoTypeItemPatch
import com.bastion.app.keepass.KeePassAutoTypePatch
import com.bastion.app.keepass.KeePassBinaryPoolItemPatch
import com.bastion.app.keepass.KeePassBinaryReferencePatch
import com.bastion.app.keepass.KeePassCustomDataPatch
import com.bastion.app.keepass.KeePassEntryFingerprint
import com.bastion.app.keepass.KeePassFieldChange
import com.bastion.app.keepass.KeePassEntryTreeSnapshot
import com.bastion.app.keepass.KeePassGroupTreeChangePatch
import com.bastion.app.keepass.KeePassGroupTreeSnapshot
import com.bastion.app.keepass.KeePassPendingChangeBaseSnapshot
import com.bastion.app.keepass.KeePassPendingChangeRepository
import com.bastion.app.keepass.KeePassPendingFlushPlan
import com.bastion.app.keepass.KeePassPendingFlushPlanner
import com.bastion.app.keepass.KeePassRemoteRebase
import com.bastion.app.keepass.KeePassStructureChangePatch
import com.bastion.app.keepass.KeePassTimesPatch
import com.bastion.app.keepass.KeePassManagedFieldScope
import com.bastion.app.keepass.KeePassEntryFieldPatch
import com.bastion.app.keepass.KeePassFieldRegistry
import com.bastion.app.keepass.KeePassPasskeySyncCodec
import com.bastion.app.keepass.KeePassTotpCodec
...
import com.bastion.app.attachments.executor.KeePassAttachmentRef
...
data class KeePassEntryData(
    ...
    /** [com.bastion.app.data.model.WifiData] 的 JSON，仅在 WIFI 条目上有值。 */
...
class KeePassKdbxService(
    ...
    companion object {
        ...
        // Keep only the active database plus a tiny LRU warm set. Bastion users may register many KDBX files.
        ...
        private const val FIELD_MONICA_LOCAL_ID = "BastionLocalId"
        private const val FIELD_MONICA_CONFLICT_COPY = "BastionConflictCopy"
        private const val FIELD_MONICA_ITEM_ID = "BastionSecureItemId"
        private const val FIELD_MONICA_ITEM_TYPE = "BastionItemType"
        private const val FIELD_MONICA_ITEM_DATA = "BastionItemData"
        private const val FIELD_MONICA_IMAGE_PATHS = "BastionImagePaths"
        private const val FIELD_MONICA_IS_FAVORITE = "BastionIsFavorite"
        ...
        private const val FIELD_MONICA_PASSKEY_CREDENTIAL_ID = "BastionkeyCredentialId"
        private const val FIELD_MONICA_PASSKEY_DATA = "BastionkeyData"
        private const val FIELD_MONICA_PASSKEY_MODE = "BastionkeyMode"
        private const val FIELD_MONICA_SSH_ALGORITHM = "BastionSshAlgorithm"
        private const val FIELD_MONICA_SSH_KEY_SIZE = "BastionSshKeySize"
        private const val FIELD_MONICA_SSH_PUBLIC_KEY = "BastionSshPublicKey"
        private const val FIELD_MONICA_SSH_PRIVATE_KEY = "BastionSshPrivateKey"
        private const val FIELD_MONICA_SSH_FINGERPRINT = "BastionSshFingerprint"
        private const val FIELD_MONICA_SSH_COMMENT = "BastionSshComment"
        private const val REMOTE_CONFLICT_TITLE_SUFFIX = "[远端冲突副本]"
        private const val FIELD_MONICA_SSH_FORMAT = "BastionSshFormat"
        ...
        // Bastion 扩展元数据（安全类型、代理、IP 等）。只有 Bastion 自己解析；
        // 其它客户端看到的是一段 JSON 字符串，不影响它们的常规使用。
        private const val FIELD_MONICA_WIFI_DATA = "BastionWifiData"
        ...
        private const val FIELD_MONICA_SSO_REF_ENTRY_ID = "BastionSsoRefEntryId"
    ...
    // ================================================================
    // Attachment API（供 com.bastion.app.attachments.executor 使用）
    //
    // 对应 .kiro/specs/bastion-android-attachments Requirement 6。
    // 所有方法都通过既有的 `mutateDatabase` / `loadDatabase` 通道，不另起 IO 路径，
    // 以保证 kdbx 保存时机、锁与缓存行为与密码/安全项一致。
    // ================================================================

    /** 附件元数据视图（供 executor 层构造 [com.bastion.app.attachments.model.Attachment] 使用）。 */
    ...
    private fun buildEntryFields(
        ...
        val bastionId = if (entry.id > 0) entry.id.toString() else ""
        ...
        if (bastionId.isNotEmpty()) {
            pairs.add(FIELD_MONICA_LOCAL_ID to EntryValue.Plain(bastionId))
        }
        appendPasswordCompatibilityFields(pairs, entry)
        // WIFI 条目：写入与 keepass2android WLan 模板兼容的标准字段（SSID + Password），
        // 并把 Bastion 专属扩展 (安全类型、代理、IP 设置等) 塞到 BastionWifiData JSON。
        // 这样其他 KeePass 客户端能读到 SSID/Password 基本信息，Bastion 自己读回时
        // 则能无损还原完整配置。
        if (entry.isWifiEntry()) {
            pairs += FIELD_MONICA_LOGIN_TYPE to EntryValue.Plain("WIFI")
            val wifi = com.bastion.app.data.model.WifiData.fromJsonOrEmpty(entry.wifiMetadata)
    ...
    private fun buildSecureItemFields(item: SecureItem): EntryFields {
        val bastionId = if (item.id > 0) item.id.toString() else ""
        ...
        if (bastionId.isNotEmpty()) {
            pairs.add(FIELD_MONICA_ITEM_ID to EntryValue.Plain(bastionId))
        }
        return EntryFields.of(*pairs.toTypedArray())
    }
    ...
    private fun matchesPasswordEntry(
        ...
        val bastionId = getFieldValue(entry, FIELD_MONICA_LOCAL_ID, resolutionContext).toLongOrNull()
        if (bastionId != null && target.id > 0 && bastionId == target.id) {
            return true
        }
    ...
    private fun matchesSecureItemEntry(
        ...
        val bastionId = getFieldValue(entry, FIELD_MONICA_ITEM_ID, resolutionContext).toLongOrNull()
        if (bastionId != null && target.id > 0 && bastionId == target.id) {
            return true
        }
    ...
    private fun analyzePasswordEntry(
        ...
        // Bastion 安全项（TOTP/笔记/卡片等）会写入 BastionItemType，不应进入密码列表。
        ...
        // WIFI 识别：优先使用 Bastion 自己写的元数据（包含安全类型/代理/IP 等），
        // 兜底看 keepass2android WLan 模板的 SSID 字段；两者都有的话 Bastion 元
        // 数据更完整，优先用它。
        val bastionLoginType = getFieldValue(entry, FIELD_MONICA_LOGIN_TYPE, resolutionContext)
        val bastionWifiJson = getFieldValue(entry, FIELD_MONICA_WIFI_DATA, resolutionContext)
        ...
        val (resolvedLoginType, resolvedWifiJson) = when {
            bastionLoginType.equals("WIFI", ignoreCase = true) && bastionWifiJson.isNotBlank() ->
                "WIFI" to bastionWifiJson
            bastionLoginType.equals("WIFI", ignoreCase = true) -> {
                val wifi = com.bastion.app.data.model.WifiData(ssid = ssidField.ifBlank { title })
                "WIFI" to wifi.toJson()
            }
            ssidField.isNotBlank() -> {
                // 外部客户端（kp2a/KeePassXC 手动填 SSID 字段）写入的兼容条目。
                val wifi = com.bastion.app.data.model.WifiData(ssid = ssidField)
                "WIFI" to wifi.toJson()
            }
            bastionLoginType.equals("SSO", ignoreCase = true) || ssoProvider.isNotBlank() -> "SSO" to ""
            bastionLoginType.equals("SSH_KEY", ignoreCase = true) -> "SSH_KEY" to ""
            else -> "PASSWORD" to ""
        }
        val data = KeePassEntryData(
            title = title,
    ...
    private fun entryToSecureItemData(
        ...
        if (typeRaw.isNotBlank()) {
            ...
            val sourceBastionId = getFieldValue(entry, FIELD_MONICA_ITEM_ID, resolutionContext).toLongOrNull()
            val now = Date()
            val inRecycleBin = resolveRecycleBinFlag(
                groupPath = groupPath,
                isInRecycleBinByMeta = isInRecycleBinByMeta,
            ...
            return KeePassSecureItemData(
                ...
                sourceBastionId = sourceBastionId,
                isInRecycleBin = inRecycleBin
            )
        }
        ...
        return KeePassSecureItemData(
            ...
            sourceBastionId = getFieldValue(entry, FIELD_MONICA_ITEM_ID, resolutionContext).toLongOrNull(),
            isInRecycleBin = inRecycleBin
        )
    }
    ...
    private fun entryToPasskey(
        ...
        if (isBastionConflictCopy(entry, resolutionContext)) {
            return null
        }
        val title = getFieldValue(entry, "Title", resolutionContext)
        val notes = getFieldValue(entry, "Notes", resolutionContext)
    ...
    private fun isBastionConflictCopy(
        entry: Entry,
        resolutionContext: KeePassEntryResolutionContext? = null
    ): Boolean {
    ...
    private suspend fun createRemoteFileSource(
        database: LocalKeePassDatabase,
        remoteSource: com.bastion.app.data.KeepassRemoteSource
