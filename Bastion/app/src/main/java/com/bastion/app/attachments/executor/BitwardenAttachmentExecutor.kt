...
import com.bastion.app.attachments.crypto.BitwardenAttachmentCrypto
...
import com.bastion.app.bitwarden.api.AttachmentUploadRequest
import com.bastion.app.bitwarden.api.AttachmentUploadResponse
 ...
 *    再用 Bastion 的 [AttachmentStorage] 格式重新加密存入 Local_Encrypted_Store。
...
class BitwardenAttachmentExecutor(
    ...
    suspend fun upload(
        ...
        // 4. 用 Bastion 的 Local_Encrypted_Store 保存一份本地缓存（便于 offline 预览）
        val localBlob = try {
            ciphertextTmp.inputStream().use { readBack ->
                // 上面已经写的是 Bitwarden 格式，为了让本地缓存与 LocalAttachmentExecutor 一致，
                // 这里重新用"明文 → Bastion GCM 密文"路径。我们需要重新解密 Bitwarden 格式
    ...
    suspend fun download(
        ...
        try {
            ...
            // 解密为明文后，按 Bastion 格式重新加密存入本地密文库
