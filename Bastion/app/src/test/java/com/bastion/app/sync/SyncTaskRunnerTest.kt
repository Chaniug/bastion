...
class SyncTaskRunnerTest {
    ...
    fun queuedNetworkGateBlockWritesCoreSyncDiagnostic() {
        val runnerSource = projectFile(
            "app/src/main/java/com/bastion/app/sync/SyncTaskRunner.kt"
        ).readText()
        ...
        assertTrue(
            "Execution-time network gate blocks must write BastionSyncDiag blocked logs so real-device traces show blocked instead of looking like a hang.",
    ...
    fun coordinatorWritesLayeredLifecycleDiagnosticsForPressureLogs() {
        val coordinatorSource = projectFile(
            "app/src/main/java/com/bastion/app/sync/DefaultSyncCoordinator.kt"
        ).readText()
