...
import com.bastion.app.utils.LegacyBastionSecureCsvRole
import com.bastion.app.utils.LegacyBastionZipCsvRestoreParser
import java.io.File

class LegacyBastionZipCsvRestoreParserTest {

    @Test
    fun cardsDocsCsvUsesDeterministicCardAndDocumentRestore() {
        val file = File.createTempFile("bastion_cards_docs", ".csv")
        file.writeText(
            """
        ...
        val result = LegacyBastionZipCsvRestoreParser.parseSecureItems(
            file = file,
            role = LegacyBastionSecureCsvRole.CARDS_DOCS_ONLY
        )
    ...
    fun notesCsvForcesItemsIntoNoteDomain() {
        val file = File.createTempFile("bastion_notes", ".csv")
        file.writeText(
            """
            ID,Type,Title,Data,Notes,IsFavorite,ImagePaths,CreatedAt,UpdatedAt,CategoryId
        ...
        val result = LegacyBastionZipCsvRestoreParser.parseSecureItems(
            file = file,
            role = LegacyBastionSecureCsvRole.NOTES_ONLY
        )
